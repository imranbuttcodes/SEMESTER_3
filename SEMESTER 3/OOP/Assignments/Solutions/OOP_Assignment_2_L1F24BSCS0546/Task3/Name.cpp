...
Name Name::operator+( Name& right) {
    Name temp;
    temp.firstName = (char*)fullName();
    temp.lastName = (char*)right.fullName();
...
{
    for (int i = 0; firstName[i] != '\0'; i++) {
        if (firstName[i] != right.firstName[i]) {
            return false;
        }
    }
    for (int i = 0; lastName[i] != '\0'; i++) {
        if (lastName[i] != right.lastName[i]) {
            return false;
        }
    }
    return true;
...
ostream& operator<<(ostream& out, Name& right) {
    right.display();
    return out;
}
...
istream& operator>>(istream& in, Name& right) {
