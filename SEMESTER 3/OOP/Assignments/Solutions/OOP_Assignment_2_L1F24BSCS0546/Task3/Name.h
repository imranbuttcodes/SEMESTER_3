...
public:
	...
	const char* fullName(); // Return concatenated full name (with space in between)
	...
	int strLen(char* arr);
	Name operator+(Name& right);
	Name operator=(const Name& right);
	bool operator==(const Name& right) const;
	bool operator!=(const Name& right) const;
	
friend ostream& operator<<(ostream& os,  Name& right);
friend istream& operator>>(istream& in, Name& right);
	~Name();
};
